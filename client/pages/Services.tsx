...
export default function Services() {
  ...
  const subscriptedServices = useMemo(() => {
    const serviceKeyArr = subscribtionInfo?.services?.map(
      (el) => el?.serviceKey,
    );
  ...
  const registerArray = useMemo(() => {
    if (!serviceList || !user?.services) return [];
    return serviceList.filter((service) =>
      user.services.includes(service.serviceKey),
    );
